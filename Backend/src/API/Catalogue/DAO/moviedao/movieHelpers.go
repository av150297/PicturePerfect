...
//SaveReview function
func (movie *Movie) SaveReview() error {
	reviewAPIURL := fmt.Sprintf(utils.REVIEWAPIURL, "movie", movie.MovieID, utils.APIKEY)
	client := http.Client{}
	...
	err := reviewList.Save(movie.MovieID, "movie")
	return err
}
...
//GetMovie function
func (movie *Movie) GetMovie(movieID int) error {
	db, dberr := gorm.Open("mysql", utils.DATABASEURL)
	if dberr != nil {
		return dberr
	}
